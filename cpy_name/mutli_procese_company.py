#coding:utf-8

from multiprocessing import Pool, cpu_count
import requests, json
from gevent.queue import Queue
from pyquery import PyQuery
import re, time, copy, json

src_path = '/Users/hulk/Workspace/PycharmProjects/data_process/dict/en_company_weak.dict'
save_path = '/Users/hulk/Workspace/PycharmProjects/data_process/dict/en_company_weak_verify.dict'
proxies_queue = Queue(100)
headers = {
    'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.102 Safari/537.36'}
cookies = {
    'cookie': 'UM_distinctid=167557452b5f6-0b4644e0763f48-35627400-13c680-167557452b6377; _ga=GA1.2.392079514.1543326947; cloud-anonymous-token=d8246fe4f0324e40aa514643feff8b75; _gid=GA1.2.1493862284.1543630829; cloud-sso-token=DEB1E1ADE4CE73FE0651C7071646607B; _gat=1'}
formerly_pat = re.compile('(?<=formerly:)[a-z0-9,.\s]+(?=\(filings)', re.I)
start_num_pat = re.compile('[0-9]+')
fw = open(save_path, 'a')

def generate_urls():
    company_name_list = set()
    with open(src_path, 'r') as fr:
        for line in fr:
            for word in line.split():
                company_name_list.add(word)

    urls = []
    for word in company_name_list:
        urls.append('https://www.sec.gov/cgi-bin/browse-edgar?company=' + word + '&owner=exclude&action=getcompany')
    return urls

def split_urls(urls):
    if not urls:
        print('no url in urls')
        return [urls]
    num_urls = len(urls)
    num_cpus = cpu_count() + 1
    if num_urls < num_cpus:
        return [urls]
    num_urls_per_cpu = int(num_urls / num_cpus)
    splitted_urls = []
    for i in range(num_cpus):
        if i == 0:
            splitted_urls.append(urls[: (i + 1) * num_urls_per_cpu])
        elif i == num_cpus - 1:
            splitted_urls.append(urls[i * num_urls_per_cpu:])
        else:
            splitted_urls.append(urls[i * num_urls_per_cpu: (i + 1) * num_urls_per_cpu])

    return splitted_urls

def proxy_producer():
        ip_url = 'http://193.112.31.15:5000/proxy'
        headers = {
            'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.110 Safari/537.36'}
        response = requests.get(ip_url, headers=headers, timeout=5)
        ips = json.loads(response.text)
        for ip in ips:
            proxies = {
                'http': 'http://' + ip,
                'https': 'http://' + ip,
            }
            if not proxies_queue.full():
                proxies_queue.put_nowait(proxies)

def parse(res):
        doc = PyQuery(res.text)
        url = res.url
        cpy_list = []

        container = doc('#contentDiv')
        # 提取公司名
        tds = container('td').items()
        for td in tds:
            if td.attr('valign'):
                continue
            html_str = td.html()
            if not html_str:
                continue
            lidx = html_str.find('<br/>')
            if lidx:
                # print(html_str[:lidx])
                cpy_list.append(html_str[:lidx])
                mat = formerly_pat.search(html_str)
                if mat:
                    # print(mat.group().strip())
                    cpy_list.append(mat.group().strip())
        if not cpy_list:
            return cpy_list

        # 检测是否还有其他页
        input_list = list(container('form').items())
        if input_list:
            click_val = input_list[-1]('input').attr('onclick')
            if click_val:
                cgi_idx = click_val.find('/cgi-bin')
                click_val = 'https://www.sec.gov' + click_val[cgi_idx:-1]
                # print(click_val)
                url_idx = url.find('start=')
                start_idx = click_val.find('start=')
                if url_idx < 0:
                    temp_url = click_val[:start_idx] + 'start=' + str(0) + '&count=100&hidefilings=0'
                else:
                    start_mat = start_num_pat.search(url[start_idx:])
                    start_num = int(start_mat.group()) + 100
                    temp_url = click_val[:start_idx] + 'start=' + str(start_num) + '&count=100&hidefilings=0'

                # self.new_urls.append(temp_url)

        return cpy_list

def crawling_web(urls):
    for url in urls:
        # try:
            if proxies_queue.empty():
                proxy_producer()
            proxies = proxies_queue.get_nowait()
            res = requests.get(url, timeout=10)
            cpy_list = parse(res)
            fw.writelines(cpy_list)
            fw.flush()
        # except:
        #     print('fali to crawle {}'.format(url))


org_urls = generate_urls()
splitted_urls = split_urls(org_urls)
cpu_num = cpu_count() + 1
pol = Pool(cpu_num)
for urls in splitted_urls:
    pol.apply_async(crawling_web, args=(urls,))

fw.close()



